...
void setup() {
  ...
  esp_err_t err = cam.init(&cam_conf, QVGA, RGB565);
  if(err != ESP_OK)   Serial.println("cam.init ERROR");

  cam.setPCLK(2, DBLV_CLK_x4);  // PCLK : 10MHz / (pre+1) * 4 --> 13.3MHz  
  cam.vflip( false );
...
void loop(void) {
  ...
  while(1){
    for( y = 0; y < 240; y++){      
      buf = cam.getLine( y+1 );               // LineNo(1~240)
      tft.drawRGBBitmap( 0, y, buf, 320, 1);  // TFT 
    }
    /*
    if( ts.touched() ){
